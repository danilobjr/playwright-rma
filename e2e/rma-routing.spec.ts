...
test('opens the RMA request list route', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'RMA Requests' })).toBeVisible()
  await expect(
    page.getByText('Track return merchandise authorizations, filter by status and date, and start new requests.'),
  ).toBeVisible()
...
test('navigates from list to create RMA request shell', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Create RMA Request' })).toBeVisible()
  await expect(breadcrumb.getByText('RMA', { exact: true })).toBeVisible()
  await expect(breadcrumb.getByText('New request')).toBeVisible()
  await expect(
    page.getByText('Use a compact FieldGroup form layout that matches shadcn form composition.'),
  ).toBeVisible()
  await expect(page.getByRole('heading', { name: 'Request details' })).toBeVisible()
  await expect(page.getByText('RMA ID is generated automatically. Status starts as Pending.')).toBeVisible()
})
...
test('opens the dynamic RMA update route placeholder', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Update RMA Status' })).toBeVisible()
  await expect(page.getByRole('navigation', { name: 'Breadcrumb' }).getByText('RMA-2026-1047')).toBeVisible()
  await expect(page.getByText('Review the request and choose the next workflow status.')).toBeVisible()
...
test('does not show the scaffold placeholder', async ({ page }) => {
  ...
  await expect(page.getByText("I'm a useless example page. You can delete me")).toHaveCount(0)
})
